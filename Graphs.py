...
ax1.set_title(chart1_title, pad=16, wrap=True)
fig1.tight_layout()
...
# ══════════════════════════════════════════════════════════════════════════════
# Convert willingness share to absolute % of all respondents
...
base_pct     = [flight_01_total, 0, 0]   # 0-1 has no willingness split
...
b3 = ax2.bar(x, unwilling_pct,bar_w, label="Not willing to change",    color=NO_COLOR,
             bottom=willing_pct, linewidth=1.2, edgecolor="white")
...
ax2.set_xticks(x)
ax2.set_xticklabels(flight_cats, fontsize=12)
ax2.set_ylabel("% of all respondents")
ax2.set_ylim(0, 80)
ax2.set_title(f"{chart2_title}\n(with willingness to use a more sustainable alternative)", pad=14)
...
ax3.set_title(chart3_title, pad=16)
fig3.tight_layout()
...
ax4_main.bar(x4, yes_bar,    bw4, color=YES_COLOR,    edgecolor="white", lw=1.2, label="Recycles / willing to change")
ax4_main.bar(x4, will_bar,   bw4, color=YES_COLOR,    edgecolor="white", lw=1.2, alpha=0.65)
ax4_main.bar(x4, nowill_bar, bw4, color=NO_COLOR,     edgecolor="white", lw=1.2,
             bottom=will_bar, label="Not willing to change")

# Labels
for val, xpos in zip([plastic_yes], [0]):
    ax4_main.text(xpos, val / 2, f"{val}%", ha="center", va="center",
...
ax4_main.set_xticklabels(cats, fontsize=11)
ax4_main.set_ylabel("% of all respondents")
ax4_main.set_ylim(0, 85)
...
ax4_main.legend(handles=legend_patches, loc="upper right", framealpha=0.9, fontsize=10)
...
wedges_d, _, autotexts_d = ax4_inset.pie(
    donut_sizes,
    labels=donut_labels,
    colors=donut_colors,
    autopct="",
    startangle=90,
    wedgeprops=dict(width=0.52, linewidth=1.4, edgecolor="white"),
...
ax4_inset.set_title("Willingness to change\namong non-recyclers", fontsize=11, pad=10)

ax4_main.set_title(chart4_title, pad=14)
